...
Panel {
    ...
    readonly property string keyboardConfigPath: Quickshell.env("HOME") + "/.config/hypr/input.lua"
    readonly property string pluginPath: Quickshell.env("HOME") + "/.config/omarchy/plugins/" + moduleName
    readonly property string legacySettingsPath: pluginPath + "/.settings.json"
    readonly property var trackerService: bar?.shell?.serviceFor(moduleName)
    readonly property string pulseColor: normalizedPulseColor(setting("pulseColor", tealColor))
    readonly property bool animationEnabled: setting("animation", true) !== false
    readonly property bool showSingleLayout: setting("showSingleLayout", false) === true
    readonly property bool perWindowLayouts: setting("perWindowLayouts", false) === true
    readonly property bool trackerSupportKnown: trackerService ? trackerService.supportKnown : false
    readonly property bool trackerSupported: trackerService ? trackerService.supported : false
    readonly property bool perWindowLayoutsActive: perWindowLayouts && trackerSupported
    ...
    function presetForColor(value) {
        ...
        return color === tealColor || color === purpleColor || color === blueColor || color === yellowColor ? color :
                                                                                                              "custom";
    }
    ...
    function loadLegacySettings(raw) {
        try {
            ...
            root.legacySettings = value && typeof value === "object" && !Array.isArray(value) ? value : null;
        } catch (error) {
    ...
    function migrateLegacySettings() {
        if (root.legacySettingsMigrated || !root.legacySettingsLoaded || !root.bar || !root.bar.shell
    ...
    function setShowSingleLayout(enabled) {
        persistSettings({
                            showSingleLayout: enabled
                        });
        if (!enabled && !root.multipleLayouts)
            root.close();
    }

    function setPerWindowLayouts(enabled) {
        persistSettings({
                            perWindowLayouts: enabled
                        });
        syncLayoutTracker();
    }

    function syncLayoutTracker() {
        if (root.trackerService && typeof root.trackerService.setTrackingEnabled === "function")
    ...
    function expectAutomaticRestore(layout) {
    ...
    function updateKeyboardConfig(raw) {
        ...
        for (var index = 0; index < lines.length; index++) {
            ...
            if (/kb_options\s*=/.test(lines[index]))
    ...
    function openSettings() {
        ...
        customColorField.text = root.pulseColor;
        root.customColorEditorVisible = root.presetForColor(root.pulseColor) === "custom";
    }
    ...
    function editKeyboardShortcut() {
        root.close();
        Quickshell.execDetached(["omarchy-launch-terminal", "nvim", "+" + String(root.keyboardOptionsLine),
                                 "+normal! zz", root.keyboardConfigPath]);
    }
    ...
    function typingKeyboards(keyboards) {
        ...
        var typing = physical.filter(function (keyboard) {
            ...
            return !name.endsWith("-system-control") && !name.endsWith("-consumer-control") && name !== "video-bus" &&
                    !name.startsWith("power-button") && !name.endsWith("-extra-buttons") && !name.endsWith(
                        "-wmi-hotkeys");
        });
    ...
    function switchLayouts(target) {
        if (!root.bar || root.keyboardNames.length === 0)
            return;
        var commands = root.keyboardNames.map(function (name) {
            return "hyprctl switchxkblayout -- " + Util.shellQuote(name) + " " + Util.shellQuote(target);
        });
    ...
    function activateCursor() {
        if (!root.cursorActive)
            return;
        if (root.cursorIndex === root.layouts.length)
            root.openSettings();
        else
            root.selectLayout(root.cursorIndex);
    }
    ...
    onAnimationEnabledChanged: if (!animationEnabled)
                                   resetPulse()
    ...
    Text {
        ...
        text: root.layoutLabel
        color: root.animationEnabled && pulseAnimation.running ? root.pulseColor : button.foreground
    ...
    KeyboardPanel {
        ...
        contentWidth: panel.fittedContentWidth(Style.space(root.settingsPage ? 260 : 180))
        contentHeight: panel.fittedContentHeight(panelColumn.implicitHeight)

        PanelKeyCatcher {
            id: keyCatcher
            anchors.fill: parent
            blocked: root.settingsPage && (colorDropdown.popupOpen || (root.customColorEditorVisible
            ...
            Column {
                ...
                Column {
                    ...
                    Button {
                        ...
                        leftAlign: true
                        bordered: true
                        hasCursor: root.cursorActive && root.cursorIndex === root.layouts.length
                ...
                Column {
                    ...
                    Item {
                        width: parent.width
                        implicitHeight: Math.max(pulseColorHeader.implicitHeight, animationToggleRow.implicitHeight)
                        ...
                        Row {
                            ...
                            PanelSectionHeader {
                                ...
                                text: "Animation"
                                foreground: root.animationEnabled ? root.bar.foreground : Color.muted
                                fontFamily: root.bar.fontFamily
                                anchors.verticalCenter: parent.verticalCenter
                            }
                            ...
                            ToggleSwitch {
                                ...
                                anchors.verticalCenterOffset: Math.round(animationToggleLabel.topPadding / 2)
                                trackHeight: Math.round(animationToggleLabel.font.pixelSize * 1.2)
                                ...
                                PanelToolTip {
                                    ...
                                    text: root.animationEnabled ? "Disable the layout-change animation" :
                    ...
                    ColorDropdown {
                        ...
                        value: root.customColorEditorVisible ? "custom" : root.presetForColor(root.pulseColor)
                        customColor: root.isPulseColor(customColorField.text) ? root.normalizedPulseColor(
                        ...
                        onChanged: function (value) {
                            if (value === "custom")
                                root.openCustomColorEditor();
                            else
                                root.selectPresetColor(value);
                        }
                    }
                    ...
                    Text {
                        visible: root.animationEnabled && root.customColorEditorVisible && customColorField.text !== ""
                    ...
                    Item {
                        width: parent.width
                        implicitHeight: Math.max(shortcutText.implicitHeight, editShortcutButton.implicitHeight)
                    ...
                    Item {
                        width: parent.width
                        implicitHeight: Math.max(singleLayoutHeader.implicitHeight, singleLayoutToggle.implicitHeight)
                        ...
                        PanelSectionHeader {
                            ...
                            foreground: root.showSingleLayout ? root.bar.foreground : Color.muted
                            fontFamily: root.bar.fontFamily
                        }
                        ...
                        ToggleSwitch {
                            ...
                            anchors.verticalCenterOffset: Math.round(singleLayoutHeader.topPadding / 2)
                            trackHeight: Math.round(singleLayoutHeader.font.pixelSize * 1.2)
                            cursorPad: Style.space(3)
                            ...
                            PanelToolTip {
                                ...
                                text: root.showSingleLayout ? "Hide when only one layout is configured" :
                    ...
                    Item {
                        width: parent.width
                        implicitHeight: Math.max(perWindowHeader.implicitHeight, perWindowToggle.implicitHeight)
                        opacity: root.trackerSupportKnown && root.trackerSupported ? 1 : 0.45
                        ...
                        PanelSectionHeader {
                            ...
                            elide: Text.ElideRight
                            foreground: root.perWindowLayoutsActive ? root.bar.foreground : Color.muted
                            fontFamily: root.bar.fontFamily
                        }
                        ...
                        ToggleSwitch {
                            ...
                            anchors.verticalCenterOffset: Math.round(perWindowHeader.topPadding / 2)
                            trackHeight: Math.round(perWindowHeader.font.pixelSize * 1.2)
                            cursorPad: Style.space(3)
                            ...
                            PanelToolTip {
                                ...
                                text: root.trackerSupported ? (root.perWindowLayoutsActive
